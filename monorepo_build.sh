...
# Final monorepo will contain all branches from the first remote and development branches of all remotes will be merged
...
for PARAM in $@; do
    ...
    # Rewrite all branches from the first remote, only development branches from others
    if [ "$PARAM" == "$1" ]; then
        ...
        MERGE_REFS='development'
    else
        echo "Building branch 'development' of the remote '$REMOTE'"
        git checkout --detach $REMOTE/development
...
# Merge all development branches
...
for REF in $MERGE_REFS; do
    # Add all files from all development branches into index
